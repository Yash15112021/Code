function H_hf = hyperfine()
    ...
    FAD = 0.5233*eye(3) *1e-3;
    FAD1 = 0.3215*eye(3)* 1e-3;
    ...
    H_hf =   FAD(1,1)*makron(S_x,eye(2),I_x,eye(3)) +FAD(2,2)*makron(S_y,eye(2),I_y,eye(3)) + FAD(3,3)*makron(S_z,eye(2),I_z,eye(3))+...
             FAD1(1,1)*makron(eye(2),S_x,eye(3),I_x)+FAD1(2,2)*makron(eye(2),S_y,eye(3),I_y) + FAD1(3,3)*makron(eye(2),S_z,eye(3),I_z);
end
